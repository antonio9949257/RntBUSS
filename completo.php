        ...
        <form method="POST" action="completo.php">
            <div class="bg-white p-8 rounded-xl shadow-sm mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-4">
                    <div><label class="block text-sm font-medium text-gray-700">Costos Fijos</label><input type="number" name="fixedCosts" value="<?= htmlspecialchars($fixedCosts > 0 ? $fixedCosts : '') ?>" class="mt-1 block w-full" placeholder="0.00" step="0.01"><p class="mt-1 text-xs text-gray-500">Ej: Alquiler, salarios, servicios básicos.</p></div>
                    <div><label class="block text-sm font-medium text-gray-700">Costo Variable (Unidad)</label><input type="number" name="variableCostPerUnit" value="<?= htmlspecialchars($variableCostPerUnit > 0 ? $variableCostPerUnit : '') ?>" class="mt-1 block w-full" placeholder="0.00" step="0.01"><p class="mt-1 text-xs text-gray-500">Ej: Materia prima, costo de producción unitario.</p></div>
                    <div><label class="block text-sm font-medium text-gray-700">Precio de Venta (Unidad)</label><input type="number" name="salePrice" value="<?= htmlspecialchars($salePrice > 0 ? $salePrice : '') ?>" class="mt-1 block w-full" placeholder="0.00" step="0.01"><p class="mt-1 text-xs text-gray-500">Precio final al que vendes cada producto.</p></div>
                    <div><label class="block text-sm font-medium text-gray-700">Unidades Vendidas</label><input type="number" name="unitsSold" value="<?= htmlspecialchars($unitsSold > 0 ? $unitsSold : '') ?>" class="mt-1 block w-full" placeholder="0"><p class="mt-1 text-xs text-gray-500">Cantidad de productos que estimas vender.</p></div>
                    <div><label class="block text-sm font-medium text-gray-700">Tasa de Impuestos (%)</label><input type="number" name="taxRate" value="<?= htmlspecialchars($taxRate > 0 ? $taxRate : '') ?>" class="mt-1 block w-full" placeholder="e.g., 21"><p class="mt-1 text-xs text-gray-500">Impuesto sobre la renta o sociedades. Ej: 21.</p></div>
                    <div><label class="block text-sm font-medium text-gray-700">Gastos Financieros</label><input type="number" name="financialExpenses" value="<?= htmlspecialchars($financialExpenses > 0 ? $financialExpenses : '') ?>" class="mt-1 block w-full" placeholder="0.00" step="0.01"><p class="mt-1 text-xs text-gray-500">Intereses de préstamos, comisiones bancarias.</p></div>
                    <div><label class="block text-sm font-medium text-gray-700">Depreciación</label><input type="number" name="depreciation" value="<?= htmlspecialchars($depreciation > 0 ? $depreciation : '') ?>" class="mt-1 block w-full" placeholder="0.00" step="0.01"><p class="mt-1 text-xs text-gray-500">Pérdida de valor de tus activos (maquinaria, etc.).</p></div>
                </div>
